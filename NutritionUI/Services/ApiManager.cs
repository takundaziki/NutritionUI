...
namespace NutritionUI.Services
{
    public class ApiManager : IApiManager
    {
        ...
        //Receive in constructor Api Service<Refit Interface>
        public ApiManager(IApiService<IItemApi> _itemApi)
        {
            itemApi = _itemApi;
...
//if there is no internet cancel all the running requests
            if (!e.IsConnected)
            {
        ...
        //calls RemoteRequestAsyncMethod aand adds the tasks to the running task list
        public async Task<HttpResponseMessage> GetItems(string query)
        {
            var cts = new CancellationTokenSource();
            var task = RemoteRequestAsync<HttpResponseMessage>(itemApi.GetApi(Priority.UserInitiated).GetItems(query));
...
//if there is no internet display a toast showing it
            if (!IsConnected)
            {
